    ...
    <div className="flex-1 bg-[#F7FFF8] min-h-screen">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center px-6 py-20">
          ...
          <Link
            href={ROUTES.HOME}
            className="mt-6 rounded-[8px] bg-[#2E7D32] px-8 py-3"
          >
        ...
        <>
          {/* Cart items list */}
          <div className="px-[16px] pt-[24px] pb-[180px]">
            {items.map((item) => {
              const imageUri = item.product.imageUrls?.[0] ?? null;
              return (
                <div
                  key={item.productId}
                  className="mb-[16px] flex flex-row overflow-hidden rounded-[12px] bg-[#F7FFF8] border border-[#EAEAEA] p-[12px]"
                >
                  {/* Product Image */}
                  <div className="h-[96px] w-[96px] shrink-0 overflow-hidden rounded-[8px] bg-white flex items-center justify-center">
                    {imageUri ? (
                    ...
                    <div className="mt-[16px] flex flex-row items-center justify-between">
                      ...
                      <div className="flex flex-row items-center gap-[12px]">
                        <button
                          ...
                          className="h-[28px] w-[28px] flex items-center justify-center rounded-[8px] bg-[#F7FFF8] border border-[#EAEAEA]"
                        ...
                        <button
                          type="button"
                          onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                          className="h-[28px] w-[28px] flex items-center justify-center rounded-[8px] border border-[#EAEAEA]"
          ...
          {/* Sticky footer */}
          <div className="fixed bottom-[88px] left-1/2 -translate-x-1/2 w-full max-w-[390px] px-[24px] bg-transparent z-20">
