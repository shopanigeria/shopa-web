...
export default function ProductDetailPage() {
  ...
  return (
    <div className="flex flex-col bg-[#F7FFF8] min-h-screen">

      {/* ── Header row: back + wishlist ── */}
      <div className="flex items-center justify-between px-[24px] pt-[48px] pb-[12px]">
        <button
          type="button"
          onClick={() => router.back()}
          className="w-[40px] h-[40px] rounded-full bg-[#D8FFDA] flex items-center justify-center"
          aria-label="Go back"
        >
        ...
        <button
          ...
          className={cn(
            "w-[40px] h-[40px] rounded-full flex items-center justify-center shadow-[0px_2px_4px_rgba(0,0,0,0.1)]",
      ...
      {/* ── Product image ── */}
      <div className="h-[280px] w-full px-[24px] bg-[#F7FFF8] shrink-0">
        {imageUri ? (
      ...
      {/* ── White details card ── */}
      <div className="-mt-6 rounded-t-[30px] bg-white flex-1 pb-[160px] border-t border-[rgba(0,0,0,0.25)]">
        <div className="px-[24px] pt-[20px]">

          {/* Name + qty stepper */}
          <div className="relative pr-[112px] mb-[10px]">
            <p className="font-jakarta font-medium text-[18px] text-[#151515] leading-normal tracking-[-0.04em]">
              {product.name}
            </p>
            {/* Quantity stepper — absolutely positioned so it doesn't affect name's height */}
            <div className="absolute top-0 right-0 flex items-center rounded-[8px] border border-[#2E7D32]">
              <button
                type="button"
                onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                className="w-[32px] h-[48px] flex items-center justify-center"
                aria-label="Decrease"
              >
          ...
          {/* Price */}
          <p className="font-satoshi text-[20px] font-semibold text-[#151515] mb-[10px] leading-[1.2]">
            {formatNaira(price)}
          </p>
              ...
              <div className="flex gap-[8px] flex-wrap">
                {["XS", "S", "M", "L", "XL"].map((size) => (
                  <button
                    ...
                    className={cn(
                      "w-[40px] h-[40px] rounded-[8px] border font-jakarta text-[13px] font-medium",
                      selectedSize === size
          ...
          <div className="flex gap-[px] mb-[4px]">
            {(["details", "reviews"] as const).map((tab) => (
              <button
                ...
                className={cn(
                  "rounded-[8px] px-[12px] py-[8px] font-jakarta text-[14px] leading-[1.26] tracking-[-0.04em]",
                  activeTab === tab
              ...
              {description.length > 200 && !showFullDescription && (
                <>
                  ...
                  <button
                    ...
                    className="font-jakarta text-[14px] font-regular text-[#2E7D32] inline"
                  >
                    Show more
                  </button>
                </>
      ...
      <div className="fixed bottom-[88px] left-0 right-0 z-40 flex justify-center pointer-events-none">
